 ...
 *  Modified by Jingwei Chen 12/03/17
...
public:
	...
	void FinishUnranking(const HeavySTPState<width, height> &s)
	{
 ...
 */
//slideDir is defined in mnpuzzle.h
//enum slideDir {
//	kLeft, kUp, kDown, kRight
//};

template <int width, int height>
static std::ostream& operator <<(std::ostream & out, const HeavySTPState<width, height> &loc)
{
	out << "(" << width << "x" << height << ")";
...
template <int width, int height>
static bool operator==(const HeavySTPState<width, height> &l1, const HeavySTPState<width, height> &l2)
{
...
template <int width, int height>
class HeavySTP : public PermutationPuzzle::PermutationPuzzleEnvironment<HeavySTPState<width, height>, slideDir> {
public:
	HeavySTP();
	HeavySTP(const std::vector<slideDir> op_order); // used to set action order
	~HeavySTP();
	void SetWeighted(bool w) { weighted = w; }
	bool GetWeighted() const { return weighted; }
	void SetWeightMode(int m) { weight_mode = m; }
	int GetWeightMode() const { return weight_mode; }
	void GetSuccessors(const HeavySTPState<width, height> &stateID, std::vector<HeavySTPState<width, height>> &neighbors) const;
	void GetActions(const HeavySTPState<width, height> &stateID, std::vector<slideDir> &actions) const;
	slideDir GetAction(const HeavySTPState<width, height> &s1, const HeavySTPState<width, height> &s2) const;
	void ApplyAction(HeavySTPState<width, height> &s, slideDir a) const;
	bool InvertAction(slideDir &a) const;
	static unsigned GetParity(HeavySTPState<width, height> &state);

	OccupancyInterface<HeavySTPState<width, height>, slideDir> *GetOccupancyInfo() { return 0; }
	double HCost(const HeavySTPState<width, height> &state1, const HeavySTPState<width, height> &state2) const;
	double HCost(const HeavySTPState<width, height> &state1) const;
	double DefaultH(const HeavySTPState<width, height> &s) const;

	double GCost(const HeavySTPState<width, height> &state1, const HeavySTPState<width, height> &state2) const;
	double GCost(const HeavySTPState<width, height> &, const slideDir &) const;
	double AdditiveGCost(const HeavySTPState<width, height> &, const slideDir &);
	bool GoalTest(const HeavySTPState<width, height> &state, const HeavySTPState<width, height> &goal) const;

	bool GoalTest(const HeavySTPState<width, height> &s) const;

	void GetStateFromPDBHash(uint64_t hash, HeavySTPState<width, height> &s,
	...
	void OpenGLDraw(const HeavySTPState<width, height> &s) const;
	void OpenGLDraw(const HeavySTPState<width, height> &l1, const HeavySTPState<width, height> &l2, float v) const;
	void OpenGLDraw(const HeavySTPState<width, height> &, const slideDir &) const { /* currently not drawing moves */ }
	void StoreGoal(HeavySTPState<width, height> &); // stores the locations for the given goal state

	/** Returns stored goal state if it is stored.**/
	HeavySTPState<width, height> Get_Goal(){
		if (!goal_stored) {
	...
	**/
	static void Create_Random_MN_Puzzles(HeavySTPState<width, height> &goal, std::vector<HeavySTPState<width, height>> &puzzle_vector, unsigned num_puzzles);
	...
	described above.
	**/
	static int read_in_mn_puzzles(const char *filename, bool first_counter, unsigned max_puzzles, std::vector<HeavySTPState<width, height>> &puzzle_vector);
	...
	**/
	...
	static HeavySTPState<width, height> Generate_Random_Puzzle();

	virtual void GetStateFromHash(HeavySTPState<width, height> &s, uint64_t hash) const;
	uint64_t GetStateHash(const HeavySTPState<width, height> &s) const;
	uint64_t GetMaxStateHash() const;
	
	bool State_Check(const HeavySTPState<width, height> &to_check);
...
private:
//	double DoPDBLookup(const HeavySTPState<width, height> &state);
...
//	std::vector<std::vector<int> > PDBkey;
	...
	HeavySTPState<width, height> goal;
};

template <int width, int height>
class GraphHeavySTPDistanceHeuristic : public GraphDistanceHeuristic {
public:
	GraphHeavySTPDistanceHeuristic(HeavySTP<width, height> &mnp, Graph *graph, int count);
	double HCost(const graphState &state1, const graphState &state2) const;
private:
	HeavySTP<width, height> puzzle;
};

//typedef UnitSimulation<HeavySTPState, slideDir, HeavySTP> PuzzleSimulation;

template <int width, int height>
HeavySTP<width, height>::HeavySTP()
{
	weighted = false;
	weight_mode = 0;
...
template <int width, int height>
HeavySTP<width, height>::HeavySTP(const std::vector<slideDir> op_order)
{
...
template <int width, int height>
void HeavySTP<width, height>::Change_Op_Order(std::vector<slideDir> op_order)
{
	operators.clear();
	ops_in_order.clear();
	...
	if (op_order.size() != 4)
	{
		fprintf(stderr, "ERROR: Not enough operators in operator sequence for construction of HeavySTP\n");
		exit(1);
	}
	...
	{
		fprintf(stderr, "ERROR: Invalid operator sequence for construction of HeavySTP\n");
		exit(1);
	}
...
template <int width, int height>
const std::string HeavySTP<width, height>::GetName(){
	std::string s = "HeavySTP(";
...
template <int width, int height>
Graph *HeavySTP<width, height>::GetGraph()
{
	Graph *g = new Graph();
...
template <int width, int height>
void HeavySTP<width, height>::StoreGoal(HeavySTPState<width, height> &s)
{
...
template <int width, int height>
bool HeavySTP<width, height>::GoalTest(const HeavySTPState<width, height> &s) const
{
	return (s == goal);
}

template <int width, int height>
void HeavySTP<width, height>::ClearGoal()
{
	if (goal_stored)
	{
		goal_stored = false;
...
template <int width, int height>
void HeavySTP<width, height>::GetSuccessors(const HeavySTPState<width, height> &stateID,
							 std::vector<HeavySTPState<width, height>> &neighbors) const
{
	neighbors.resize(0);
...
template <int width, int height>
void HeavySTP<width, height>::GetActions(const HeavySTPState<width, height> &stateID, std::vector<slideDir> &actions) const
{
	actions.resize(0);
	for (unsigned int i = 0; i < operators[stateID.blank].size(); i++)
	{
...
template <int width, int height>
slideDir HeavySTP<width, height>::GetAction(const HeavySTPState<width, height> &a, const HeavySTPState<width, height> &b) const
{
...
template <int width, int height>
void HeavySTP<width, height>::ApplyAction(HeavySTPState<width, height> &s, slideDir a) const
{
...
template <int width, int height>
bool HeavySTP<width, height>::InvertAction(slideDir &a) const
{
	switch (a)
	{
...
template <int width, int height>
double HeavySTP<width, height>::HCost(const HeavySTPState<width, height> &state1) const
{
	return DefaultH(state1);
}

template <int width, int height>
double HeavySTP<width, height>::HCost(const HeavySTPState<width, height> &state1, const HeavySTPState<width, height> &state2) const
{
	if (goal_stored)
		return HCost(state1);
...
//	}
	...
	if (use_manhattan)
	{
		...
		for (unsigned int x = 0; x < width; x++)
		{
			for (unsigned int y = 0; y < height; y++)
			{
				int tile = state1.puzzle[x + y*width];
				if (tile != 0)
				{
					if (weight_mode >= 0)
					{
						...
						for (int i = 0; i < weight_mode; i++)
							factor = factor*tile;
						man_dist += factor*(abs((int)(xloc[state1.puzzle[x + y*width]] - x))
							+ abs((int)(yloc[state1.puzzle[x + y*width]] - y)));
					}
					else //weight_mode<0
					{
						double factor = 1;
						for (int i = weight_mode; i < 0; i++)
							factor = factor*(1.0 / (double)tile);
						man_dist += factor*(abs((int)(xloc[state1.puzzle[x + y*width]] - x))
							+ abs((int)(yloc[state1.puzzle[x + y*width]] - y)));
					}
...
template <int width, int height>
double HeavySTP<width, height>::DefaultH(const HeavySTPState<width, height> &state) const
{
	double man_dist = 0;
...
template <int width, int height>
double HeavySTP<width, height>::AdditiveGCost(const HeavySTPState<width, height> &s, const slideDir &d)
{
	int tile;
	switch (d)
	{
...
template <int width, int height>
double HeavySTP<width, height>::GCost(const HeavySTPState<width, height> &a, const HeavySTPState<width, height> &b) const
{
	//	int diff = a.blank - b.blank;
	//
	int tile;

	if (a.blank > b.blank)//moving left or up
	{
		tile = a.puzzle[a.blank - b.blank];
	}
	else
	{
		tile = a.puzzle[b.blank - a.blank];
	}
	if (tile <= 0)
		return 0;
...
template <int width, int height>
double HeavySTP<width, height>::GCost(const HeavySTPState<width, height> &s, const slideDir &d) const
{
	//	double cost;
	//	switch (d)
	//	{
...
template <int width, int height>
bool HeavySTP<width, height>::GoalTest(const HeavySTPState<width, height> &state, const HeavySTPState<width, height> &theGoal) const
{
	return (state == theGoal);
}

template <int width, int height>
uint64_t HeavySTP<width, height>::GetActionHash(slideDir act) const
{
	switch (act)
	{
		case kUp: return 0;
...
template <int width, int height>
void HeavySTP<width, height>::GetStateFromPDBHash(uint64_t hash, HeavySTPState<width, height> &s,
								   int count, const std::vector<int> &pattern,
								   std::vector<int> &dual)
{
...
template <int width, int height>
void HeavySTP<width, height>::OpenGLDraw() const
{
}


template <int width, int height>
void HeavySTP<width, height>::OpenGLDraw(const HeavySTPState<width, height> &s) const
{
...
template <int width, int height>
void HeavySTP<width, height>::OpenGLDraw(const HeavySTPState<width, height> &s1, const HeavySTPState<width, height> &s2, float v) const
{
	glEnable(GL_LINE_SMOOTH);
	for (unsigned int y = 0; y < height; y++)
	{
...
/**
 Reads in HeavySTP states from the given filename. Each line of the
 ...
 **/
template <int width, int height>
int HeavySTP<width, height>::read_in_mn_puzzles(const char *filename, bool puzz_num_start, unsigned int max_puzzles,
												std::vector<HeavySTPState<width, height>> &puzzles)
{
	
	std::vector<std::vector<int> > permutations;
	PermutationPuzzle::PermutationPuzzleEnvironment<HeavySTPState<width, height>, slideDir>::Read_In_Permutations(filename, width*height, max_puzzles, permutations, puzz_num_start);
	
	// convert permutations into HeavySTPStates
	for (unsigned i = 0; i < permutations.size(); i++)
	{
		HeavySTPState<width, height> new_state;
...
template <int width, int height>
GraphHeavySTPDistanceHeuristic<width, height>::GraphHeavySTPDistanceHeuristic(HeavySTP<width, height> &mnp, Graph *graph, int count)
...
template <int width, int height>
double GraphHeavySTPDistanceHeuristic<width, height>::HCost(const graphState &state1, const graphState &state2) const
{
	HeavySTPState<3, 3> a, b;
 ...
 **/
template <int width, int height>
HeavySTPState<width, height> HeavySTP<width, height>::Generate_Random_Puzzle()
{
	HeavySTPState<width, height> new_puzz;
	std::vector<int> permutation = PermutationPuzzle::PermutationPuzzleEnvironment<HeavySTPState<width, height>, slideDir>::Get_Random_Permutation(width*height);
...
template <int width, int height>
uint64_t HeavySTP<width, height>::GetStateHash(const HeavySTPState<width, height> &s) const
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		for (int j = i + 1; j < size; j++)
			if (s.puzzle[j] < s.puzzle[i])
				cr++;

		hash += cr*PermutationPuzzle::PermutationPuzzleEnvironment<HeavySTPState<width, height>, slideDir>::Factorial(size - 1 - i);
	}
	return hash;
}

template <int width, int height>
void HeavySTP<width, height>::GetStateFromHash(HeavySTPState<width, height> &s, uint64_t hash) const
{
	...
	for (int i = 0; i < size; i++)
	{
		countRight = r / PermutationPuzzle::PermutationPuzzleEnvironment<HeavySTPState<width, height>, slideDir>::Factorial(size - 1 - i);
		r = r%PermutationPuzzle::PermutationPuzzleEnvironment<HeavySTPState<width, height>, slideDir>::Factorial(size - 1 - i);
		s.puzzle[i] = pz[countRight];
...
template <int width, int height>
uint64_t HeavySTP<width, height>::GetMaxStateHash() const
{
	int val = width*height;
	return PermutationPuzzle::PermutationPuzzleEnvironment<HeavySTPState<width, height>, slideDir>::Factorial(val)/2;
}
...
template <int width, int height>
unsigned HeavySTP<width, height>::GetParity(HeavySTPState<width, height> &state)
{
...
template <int width, int height>
void HeavySTP<width, height>::Create_Random_MN_Puzzles(HeavySTPState<width, height> &goal, std::vector<HeavySTPState<width, height>> &puzzle_vector, unsigned num_puzzles)
{
	std::map<uint64_t, uint64_t> puzzle_map; // used to ensure uniqueness
	
	HeavySTP my_puzz(width, height);
	...
	{
		HeavySTPState<width, height> next = Generate_Random_Puzzle(width, height);
...
template <int width, int height>
bool HeavySTP<width, height>::State_Check(const HeavySTPState<width, height> &to_check)
{
...
template <int width, int height>
std::vector<slideDir> HeavySTP<width, height>::Get_Op_Order_From_Hash(int order_num)
{
