...
public:
	BDFFOpenClosedData() {}
	BDFFOpenClosedData(const state &theData, double gCost, double hCost, double reverseHCost, uint64_t parent, uint64_t openLoc, stateLocation location)
...
public:
	...
	~BDFFOpenClosed();
	void Reset();
	uint64_t AddOpenNode(const state &val, uint64_t hash, double g, double h,double revH, uint64_t parent=kTBDFFNoNode, stateLocation whichQueue = kOpenWaiting);
...
void BDFFOpenClosed<state, CmpKey0, CmpKey1, dataStructure>::Reopen(uint64_t objKey)
{
 ...
 */
template<typename state, typename CmpKey0, typename CmpKey1,   class dataStructure>
uint64_t BDFFOpenClosed<state, CmpKey0, CmpKey1,   dataStructure>::AddOpenNode(const state &val, uint64_t hash, double g, double h,double revH, uint64_t parent, stateLocation whichQueue)
{
	...
	{
		elements.push_back(dataStructure(val, g, h,revH, parent, priorityQueues[0].size() , kOpenReady));
		
	}
	else if (whichQueue == kOpenWaiting)
	{
		elements.push_back(dataStructure(val, g, h,revH, parent, priorityQueues[1].size(), kOpenWaiting));
	}
 ...
 */
template<typename state, typename CmpKey0, typename CmpKey1,   class dataStructure>
void BDFFOpenClosed<state, CmpKey0, CmpKey1,   dataStructure>::KeyChanged(uint64_t val)
{
//	EqKey eq;
...
template<typename state, typename CmpKey0, typename CmpKey1, class dataStructure>
void BDFFOpenClosed<state, CmpKey0, CmpKey1, dataStructure>::Remove(uint64_t val)
{
...
// */
//template<typename state, typename CmpKey0, typename CmpKey1,   class dataStructure>
//void BDFFOpenClosed<state, CmpKey0, CmpKey1,   dataStructure>::IncreaseKey(uint64_t val)
//{
////	EqKey eq;
////	assert(eq(waitingQueue[table[val]], val));
////	waitingQueue[table[val]] = val;
...
template<typename state, typename CmpKey0, typename CmpKey1,   class dataStructure>
stateLocation BDFFOpenClosed<state, CmpKey0, CmpKey1,   dataStructure>::Lookup(uint64_t hashKey, uint64_t &objKey) const
{
 ...
 */
template<typename state, typename CmpKey0, typename CmpKey1,   class dataStructure>
uint64_t BDFFOpenClosed<state, CmpKey0, CmpKey1, dataStructure>::Peek(stateLocation whichQueue) const
{
	if (whichQueue == kOpenReady)
	{
 ...
 */
template<typename state, typename CmpKey0, typename CmpKey1,   class dataStructure>
inline const dataStructure &BDFFOpenClosed<state, CmpKey0, CmpKey1, dataStructure>::PeekAt(stateLocation whichQueue) const
{
	if (whichQueue == kOpenReady)
	{
		assert(OpenReadySize() != 0);
 ...
 */
template<typename state, typename CmpKey0, typename CmpKey1,   class dataStructure>
uint64_t BDFFOpenClosed<state, CmpKey0, CmpKey1,   dataStructure>::Close()
{
	assert(OpenReadySize() != 0);
...
template<typename state, typename CmpKey0, typename CmpKey1, class dataStructure>
uint64_t BDFFOpenClosed<state, CmpKey0, CmpKey1, dataStructure>::PutToReady()
{
	assert(OpenWaitingSize() != 0);
 ...
 */
template<typename state, typename CmpKey0, typename CmpKey1,   class dataStructure>
bool BDFFOpenClosed<state, CmpKey0, CmpKey1,   dataStructure>::HeapifyUp(unsigned int index, stateLocation whichQueue)
{
...
template<typename state, typename CmpKey0, typename CmpKey1,   class dataStructure>
void BDFFOpenClosed<state, CmpKey0, CmpKey1,   dataStructure>::HeapifyDown(unsigned int index, stateLocation whichQueue)
{
