...
template<typename state, typename CmpKey0, class dataStructure = BDOpenClosedData<state> >
class BDBuckets {
public:
	BDBuckets();
	~BDBuckets();
...
template<typename state, typename CmpKey0, class dataStructure>
uint64_t BDBuckets<state, CmpKey0, dataStructure>::AddOpenNode(const state &val, uint64_t hash, double g, double h, uint64_t parent, stateLocation whichQueue)
{
}
