...
//2
			...
			else//balance == 0
			{
				...
				{
					//previous we did a bunch of forward search and forward openReady is getting larger
					//we should switch to backward search
					if (lastBalance<0 && lastBalance+forwardQueue.OpenReadySize()/2>0
						&& fgreater(2 * forwardQueue.PeekAt(kOpenReady).g, CLowerBound))
					{
				...
				{
					//previous we did a bunch of backward search and backward openReady is getting larger
					//we should switch to forward search
					if (lastBalance>0 &&  backwardQueue.OpenReadySize() / 2 - lastBalance >0
						&& fgreater(2 * backwardQueue.PeekAt(kOpenReady).g, CLowerBound))
					{
						balance = 0 - forwardQueue.OpenReadySize() / 2;
						lastBalance = balance;
						SetForward(nextForward, nextBackward);
