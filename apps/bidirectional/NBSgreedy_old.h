...
struct NBSgreedyCompareOpenReady {
...
template <class state>
struct NBSgreedyCompareOpenWaiting {
	bool operator()(const BDOpenClosedData<state> &i1, const BDOpenClosedData<state> &i2) const
	{
		double f1 = i1.g + i1.h;
		double f2 = i2.g + i2.h;
...
template <class state, class action, class environment,  class priorityQueue = BDOpenClosed<state, NBSgreedyCompareOpenReady<state>, NBSgreedyCompareOpenWaiting<state>>>
class NBSgreedy {
public:
	NBSgreedy()
	{
...
void NBSgreedy<state, action, environment, priorityQueue>::GetPath(environment *env, const state& from, const state& to,
			 Heuristic<state> *forward, Heuristic<state> *backward, std::vector<state> &thePath)
{
...
template <class state, class action, class environment, class priorityQueue>
bool NBSgreedy<state, action, environment, priorityQueue>::InitializeSearch(environment *env, const state& from, const state& to,
...
template <class state, class action, class environment, class priorityQueue>
bool NBSgreedy<state, action, environment, priorityQueue>::ExpandAPair(std::vector<state> &thePath)
{
...
template <class state, class action, class environment, class priorityQueue>
bool NBSgreedy<state, action, environment, priorityQueue>::DoSingleSearchStep(std::vector<state> &thePath)
{
	ExpandAPair();
}
...
template <class state, class action, class environment, class priorityQueue>
void NBSgreedy<state, action, environment, priorityQueue>::Expand(priorityQueue &current,
														   priorityQueue &opposite,
														   Heuristic<state> *heuristic, const state &target)
{
	expand = false;
...
template <class state, class action, class environment, class priorityQueue>
void NBSgreedy<state, action, environment, priorityQueue>::OpenGLDraw() const
{
...
template <class state, class action, class environment, class priorityQueue>
void NBSgreedy<state, action, environment, priorityQueue>::OpenGLDraw(const priorityQueue &queue) const
{
...
template <class state, class action, class environment, class priorityQueue>
void NBSgreedy<state, action, environment, priorityQueue>::RebucketNodes()
{
	if (greedyed)//pull nodes from buckets
	{
...
template <class state, class action, class environment, class priorityQueue>
void NBSgreedy<state, action, environment, priorityQueue>::greedyVC()
{

}

template <class state, class action, class environment, class priorityQueue>
void NBSgreedy<state, action, environment, priorityQueue>::ProveUnidirectionally(std::vector<state> &thePath)
{
	int fcount = 0;
	int bcount = 0;
	for (uint64_t i = 0; i < forwardQueue.size(); i++)
