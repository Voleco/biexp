...
//		map = new Map("/Users/nathanst/hog2/maps/dao/brc203d.map");
		...
		forward.SetWeight(WEIGHT / 100.0);
	}
...
void StepAlgorithms()
{
	for (int x = 0; x < gStepsPerFrame; x++)
	{
		if (forwardSearchRunning)
		{
			...
			if (!forwardSearchRunning)
			{
				printf("foward A*: %llu nodes expanded cost %1.1f\n", forward.GetNodesExpanded(), me->GetPathLength(forwardPath));
	...
	for (int x = 0; x < gStepsPerFrame/2; x++)
	{
		if (mmSearchRunning)
		{
			...
			if (!mmSearchRunning)
				printf("NBSFF: %llu nodes expanded\n", nbs.GetNodesExpanded());
		}
	}
	for (int x = 0; x < gStepsPerFrame; x++)
	{
...
void AnalyzeWeighted(const char *scenario, double wt)
{
	NBS<xyLoc, tDirection, MapEnvironment> nbs;
	TemplateAStar<xyLoc, tDirection, MapEnvironment> forwardAStar;
	//forwardAStar.SetWeight(WEIGHT/100.0);
	forwardAStar.SetWeight(wt);
	TemplateAStar<xyLoc, tDirection, MapEnvironment> backwardAStar;
	//backwardAStar.SetWeight(WEIGHT/100.0);
	...
	for (int x = s.GetNumExperiments() - 1; x >= 0; x--)
	{
		...
		std::cout << "nodes:\t"
			...
			<< backwardAStar.GetNodesExpanded() << "\t"
			<< backwardAStar.GetNecessaryExpansions() << "\t"
			//<< nbs.GetNodesExpanded() 
			<< "\n";
