...
public:
	bool GetNextPair(uint64_t &nextForward, uint64_t &nextBackward)
	{
		...
		//at this point we know both queue have some elements
		...
		while (true)
		{
			...
			//otherwise both waiting queues are not empty
			...
			if (!fgreater(i3.g + i3.h, gBound))
			{
				forwardQueue.PutToReady();
			}

			const auto i4 = backwardQueue.PeekAt(kOpenWaiting);
			if (!fgreater(i4.g + i4.h, gBound))
			{
				backwardQueue.PutToReady();
			}
