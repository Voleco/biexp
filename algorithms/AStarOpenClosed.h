...
public:
	AStarOpenClosedData() {}
	AStarOpenClosedData(const state &theData, double gCost, double hCost, uint64_t parent, uint64_t openLoc, dataLocation location, double p)
	:data(theData), g(gCost), h(hCost), parentID(parent), openLocation(openLoc), where(location),potential(p) { reopened = false; }
	state data;
	double g;
	double h;
	uint64_t parentID;
...
public:
	...
	uint64_t AddOpenNode(const state &val, uint64_t hash, double g, double h, double p=0, uint64_t parent=kTAStarNoNode);
	uint64_t AddClosedNode(state &val, uint64_t hash, double g, double h, double p=0, uint64_t parent=kTAStarNoNode);
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
uint64_t AStarOpenClosed<state, CmpKey, dataStructure>::AddOpenNode(const state &val, uint64_t hash, double g, double h, double p, uint64_t parent)
{
	...
	elements.push_back(dataStructure(val, g, h, parent, theHeap.size(), kOpenList,p));
 ...
 */
template<typename state, typename CmpKey, class dataStructure>
uint64_t AStarOpenClosed<state, CmpKey, dataStructure>::AddClosedNode(state &val, uint64_t hash, double g, double h, double p,uint64_t parent)
{
	// should do lookup here...
	assert(table.find(hash) == table.end());
	elements.push_back(dataStructure(val, g, h, parent, 0, kClosedList,p));
	if (parent == kTAStarNoNode)
		elements.back().parentID = elements.size()-1;
	table[hash] = elements.size()-1; // hashing to element list location
	return elements.size()-1;
