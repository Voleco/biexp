...
public:
	...
	void BuildDataStructures()
	{
		...
		*/
#ifdef USE_PEASTAR
		for (auto x = astarf.nodeCounts.begin(); x != astarf.nodeCounts.end(); x++)
		{
			if (fless(x->first.first, optCost))
			{
				auto i2 = m_f.find(x->first.second);
				if (i2 == m_f.end())
				{
					m_f.insert({ x->first.second, x->second });
				}
				else {
					i2->second = i2->second + x->second;
				}
			}
		}
		int fCount = astarf.GetUniqueNodesExpanded();
#else
		...
		int fCount = astarf.GetNodesExpanded();
#endif // USE_PEASTAR

		astarf.InitializeSearch(e, start, goal, path);
		if (m_f.size() == 0)
		{
		...
		*/
...
#ifdef USE_PEASTAR
		for (auto x = astarb.nodeCounts.begin(); x != astarb.nodeCounts.end(); x++)
		{
			if (fless(x->first.first, optCost))
			{
				auto i2 = m_b.find(x->first.second);
				if (i2 == m_b.end())
				{
					m_b.insert({ x->first.second, x->second });
				}
				else {
					i2->second = i2->second + x->second;
				}
		...
		int bCount = astarb.GetUniqueNodesExpanded();
#else
		...
		int bCount = astarb.GetNodesExpanded();
#endif // USE_PEASTAR

		astarb.InitializeSearch(e, goal, start, path);
		...
		{
			printf("total: Forward/Backward:\t%d\t%d\t", fCount, bCount);
...
#ifdef USE_PEASTAR
	PEAStar<state, action, environment> astarf;
	PEAStar<state, action, environment> astarb;
#else
