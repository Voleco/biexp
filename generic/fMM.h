...
struct fMMCompare {
	fMMCompare(double f=0.5) { frac = f; }
	bool operator()(const AStarOpenClosedData<state> &i1, const AStarOpenClosedData<state> &i2) const
	{
		double p1 = std::max(frac*(i1.g+i1.h), i1.g*2+epsilon);
		double p2 = std::max(frac*(i2.g+i2.h), i2.g*2+epsilon);
//		double p1 = i1.g+i1.h;
//		double p2 = i2.g+i2.h;
		if (fequal(p1, p2))
		{
...
template <class state, class action, class environment, class priorityQueue = AStarOpenClosed<state, fMMCompare<state>> >
class fMM {
public:
	fMM(double f, double epsilon = 0.0) :epsilon(epsilon) { forwardHeuristic = 0; backwardHeuristic = 0; env = 0; ResetNodeCount(); }
	virtual ~fMM() {}
