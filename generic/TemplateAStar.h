...
bool TemplateAStar<state,action,environment,openList>::InitializeSearch(environment *_env, const state& from, const state& to, std::vector<state> &thePath)
{
	...
	openClosedList.AddOpenNode(start, env->GetStateHash(start), 0, weight*theHeuristic->HCost(start, goal), weight*theHeuristic->HCost(start, goal));
...
//	}
	...
	if ((stopAfterGoal) && (env->GoalTest(openClosedList.Lookup(nodeid).data, goal)))
	{
		...
		goalPotential = openClosedList.Lookup(nodeid).potential; 
		return true;
	}
...
//					std::cout << " \n";
					if (useBPMX)
					{
						openClosedList.AddOpenNode(neighbors[x],
							...
							std::max(openClosedList.Lookup(nodeid).potential, openClosedList.Lookup(nodeid).g + edgeCosts[x] + weight*theHeuristic->HCost(neighbors[x], goal)),
												   nodeid);
					}
//					if (loc == -1)
//					{ // duplicate edges
...
uint64_t TemplateAStar<state, action,environment,openList>::GetNecessaryExpansions() const
{
	...
	else
	{
		...
		{
			const auto &data = openClosedList.Lookat(x);
			if (fless(data.potential, goalPotential))
				n++;
		}
		return n;
	}
